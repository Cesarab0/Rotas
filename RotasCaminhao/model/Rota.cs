...
namespace Model
{
    public class Rota
    {
        ...
        public static void Add(Rota rota) {
            Rotas.Add(rota);
            if (Controller.Caminhao.total_rotas(rota.Caminhao.Id) != 0) {
                rota.Caminhao.Faturamento++;
            }
             if(Controller.Caminhao.total_ganho(rota.Caminhao.Id) != 0) {
                rota.Caminhao.Total_valor += rota.Valor;
            }
        }
